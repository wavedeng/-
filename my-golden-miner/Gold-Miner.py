...
class Chain:
    def __init__(self,x,y):
        self.chain_thrown_out = False
        self.chain_thrown_out_away = False
        self.chain_thrown_out_catchsomething = False
        self.chain_distance = 30
        self.radins = 0
        self.direction_left_to_right = False
        self.weight_item_caught = 0
        self.chainhead = gamebox.from_image(200, 200, sheet1[frame])
        self.offsetX = x - 500
        self.offsetY = y -75

        self.value_caught = 0

        self.directionX = self.offsetX/math.sqrt(self.offsetX*self.offsetX+self.offsetY*self.offsetY)
        self.directionY = self.offsetY/math.sqrt(self.offsetX*self.offsetX+self.offsetY*self.offsetY)
    ...
    def update(self):
        ...
        global popped_up_word_counterdown

        global money

        global frame

        self.chainhead.image = sheet1[frame]

        global value_caught


        if self.chain_thrown_out == False:
            if popped_up_word_counterdown >= 16:
                if self.direction_left_to_right == True:
                    self.radins += 3
                else:
                    self.radins -= 3
            ...
            # chain head displays
            self.chainhead.x = 500 + math.sin(self.radins / 57.29) * 75
            self.chainhead.y = 75 + math.cos(self.radins / 57.29) * 75

            # self.chainhead.x = offsetX

            camera.draw(self.chainhead)

            # chains display
            item = gamebox.from_color(500, 75, "black", 5, 5)
            for i in range(0, 25):
                item = gamebox.from_color(500 + math.sin(self.radins / 57.29) * 2.5 * i,
                                          75 + math.cos(self.radins / 57.29) * 2.5 * i, "black", 5, 5)
                camera.draw(item)

        if self.chain_thrown_out == True and self.chain_thrown_out_away == True:
            self.chain_distance += speed
            for i in range(1, self.chain_distance):
                item = gamebox.from_color(500 + self.directionX * 2.5 * i,
                                          75 + self.directionY * 2.5 * i, "black", 5, 5)
                camera.draw(item)
            self.chainhead.x = 500 + self.directionX * (10 + 2.5 * self.chain_distance)
            self.chainhead.y = 75 + self.directionY * (10 + 2.5 * self.chain_distance)
            camera.draw(self.chainhead)

        if self.chain_thrown_out == True and self.chain_thrown_out_away == False:
            self.chain_distance -= self.weight_item_caught
            for i in range(1, self.chain_distance):
                item = gamebox.from_color(500 + self.directionX* 2.5 * i,
                                          75 + self.directionY * 2.5 * i, "black", 5, 5)
                camera.draw(item)
        ...
        if self.chainhead.x < 0 or self.chainhead.x > 1000 or self.chainhead.y > 700:
            self.chain_thrown_out_away = False

        if self.chain_distance <= 29 and self.chain_thrown_out == True:
            if self.chain_thrown_out_catchsomething == True:
                if self.value_caught != 0:
                    popped_up_word_counterdown = 1
                    value_caught += self.value_caught
                money += self.value_caught
                self.chain_thrown_out_catchsomething = False
            self.chain_thrown_out = False
            character.scale_by(0.833)
            frame2 = 0  # prevent "out of range" error
            self.weight_item_caught = speed + 5

        if self.chain_thrown_out_catchsomething == True:
            item = gamebox.from_image(500 + self.directionX * 2.5 * (self.chain_distance+10),
                                      75 + self.directionY * 2.5 * (self.chain_distance+10), picture_list[self.pict_index])
            camera.draw(item)
...
def tick(keys):
    ...
    if scene == 2:
        ...
        # if pygame.K_DOWN in keys  and popped_up_word_counterdown >= 16:
            value_caught = 0
            for chain in chains:
                chain.chain_thrown_out = True
                chain.chain_thrown_out_away = True
                chain.chain_thrown_out_catchsomething = False
                chain.chain_distance = 30
 ...
 # 4-golds processing
        ...
        for chain in chains:
            for gold in gold_middle:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    chain.chain_thrown_out_away = False
                    chain.chain_thrown_out_catchsomething = True
                    chain.weight_item_caught = speed - 2
                    chain.pict_index = 1
                    chain.value_caught = 200
        ...
        for chain in chains:
            for gold in gold_small:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    chain.chain_thrown_out_away = False
                    chain.chain_thrown_out_catchsomething = True
                    chain.weight_item_caught = speed
                    chain.pict_index = 0
                    if item_gold_modifer == True:
                        chain.value_caught = 125 + 25 * level
                    else:
                        chain.value_caught = 75
        ...
        for chain in chains:
            for gold in gold_large:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    chain.chain_thrown_out_away = False
                    chain.chain_thrown_out_catchsomething = True
                    chain.weight_item_caught = speed - 4
                    chain.pict_index = 2
                    chain.value_caught = 500
            ...
            for gold in gold_dimaond:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    ...
                    if item_polisher == True:
                        chain.value_caught = 900 + 75 * level
                    else:
                        chain.value_caught = 600 + 50 * level
        ...
        for chain in chains:
            for gold in gold_small_rock:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    ...
                    chain.weight_item_caught = speed - 2
                    chain.pict_index = 3
                    chain.value_caught = 20
                    if item_rocks == True:
                        chain.value_caught = value_caught * 4
        ...
        for chain in chains:
            for gold in gold_big_rock:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    ...
                    chain.weight_item_caught = speed - 4
                    chain.pict_index = 4
                    chain.value_caught = 60
                    if item_rocks == True:
                        chain.value_caught = chain.value_caught * 4
        ...
        for chain in chains:
            for gold in gold_random_big:
                if gold.touches(chain.chainhead) and chain.chain_thrown_out_catchsomething == False:
                    if chain.chain_thrown_out_catchsomething == False:
                        chain.weight_item_caught = speed + random.randint(-4, 5)
                    chain.chain_thrown_out_away = False
                    chain.chain_thrown_out_catchsomething = True
                    chain.pict_index = 6
                    if item_luck == True:
                        chain.value_caught = random.randint(500, 1000)
                    else:
                        chain.value_caught = random.randint(-300, 700)
...
def level_generation(level):
    ...
    if level >= 8:
        for c in range(0, random.randint(2,3)):
            ...
            chains.append(Chain(item.x,item.y))
